...
export type ViewState = 'dashboard' | 'sales_order' | 'direct_sale' | 'delivery_collection' | 'customers' | 'stock_inventory' | 'daily_sales_report' | 'whatsapp_marketing' | 'eye_test' | 'companies' | 'users' | 'branches' | 'payments' | 'reports' | 'settings' | 'license';
      ...
      <header className="flex items-center justify-between px-6 py-3 bg-[#0F172A] border-b border-white/10 shrink-0">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="text-[10px] font-bold text-white/40 flex items-center gap-2 tracking-widest uppercase">
            ...
            <span>OPERATOR: {session?.role?.replace('_', ' ') || 'USER'}</span>
          </div>
          <button
            onClick={logout}
            className="p-1.5 bg-red-950/60 hover:bg-red-900 border border-red-800/80 rounded-lg text-[11px] font-semibold text-red-300 transition-all flex items-center gap-1.5 active:scale-95"
            title="Log Out"
          >
            <LogOut size={14} />
          </button>
