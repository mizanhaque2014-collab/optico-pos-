...
export function BranchSelector() {
  ...
  const loadBranches = async () => {
    ...
    try {
      ...
      const companyBranches: any[] = allBranches.filter((b: any) => 
      ...
      if (companyBranches.length > 0 && (!session?.branchID || session?.branchID === 'ALL')) {
        switchBranch(companyBranches[0].BranchID || companyBranches[0].id, companyBranches[0].BranchName || companyBranches[0].branchName);
      } else if (!session?.branchName && session?.branchID && session?.branchID !== 'ALL') {
        const current = companyBranches.find(b => b.BranchID === session.branchID || b.id === session.branchID);
        if (current) {
          switchBranch(session.branchID, current.BranchName || current.branchName);
        }
      }
    } catch (e) {
  ...
  return (
    <div className="mb-4 bg-[#0F172A] border border-white/10 rounded-xl p-3 flex flex-col gap-2">
      ...
      <select 
        ...
        onChange={(e) => {
          const b = branches.find(br => (br.BranchID === e.target.value || br.id === e.target.value));
