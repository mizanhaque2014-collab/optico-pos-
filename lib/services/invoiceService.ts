...
function mapInvoiceToPascalCase(invoice: any) {
  return {
    ...
    CompanyID: (!invoice.companyId || String(invoice.companyId).trim() === '') ? (!invoice.CompanyID || String(invoice.CompanyID).trim() === '' ? 'COMP-default' : invoice.CompanyID) : invoice.companyId,
