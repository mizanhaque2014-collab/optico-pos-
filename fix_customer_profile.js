...
code = code.replace(/\{directSaleInvoices\.map\(inv => \([\s\S]*?\)\)\}/, directSaleReplacement);
code = code.replace(/\{salesOrders\.map\(inv => \([\s\S]*?\)\)\}/, salesOrdersReplacement);
        ...
        <div className="fixed inset-0 z-[100] bg-black/85 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#0F172A] border border-white/10 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
            ...
            <div className="p-6 overflow-y-auto space-y-6 custom-scrollbar">
              <PrescriptionSection 
                customer={customer} 
                prescriptions={[viewingPrescription]} 
                onNewPrescription={() => {}} 
              />
            </div>
          </div>
